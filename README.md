...
An **LED configuration** is the combination of pixel format, component order, and output profile used for
an LED product.
...
### 6.1. Fields
 ...
 - **Transaction ID**: A 16-bit unsigned integer, little-endian, set by the host. `0x0000` selects
   fire-and-forget operation, for which the device MUST NOT send a success or `ERROR` response. A
   nonzero value requests a result, and the device MUST echo that value in the corresponding success
   or `ERROR` response. Fire-and-forget requests provide no confirmation or error reporting.
   Identifier lifetime, timeout, reuse, and duplicate rules are defined in
